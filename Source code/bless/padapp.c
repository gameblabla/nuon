...
/*
  ...
  Given the signed blessings section for an application, pad to correct length
*/
        ...
        if (coff) {
          ...
          if (!result) {
            ...
            if (bytesRead == fstat.st_size) {
              for (i = fstat.st_size; i < APP_BLESS_SECTION_LENGTH; i++) {
                buf[i] = 0;
              }
              bytesWritten = fwrite(buf, APP_BLESS_SECTION_LENGTH, 1, out);
            }
          }
          else {
